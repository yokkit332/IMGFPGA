...
module uart_rx_fsm #(
    ...
    assign shared_sample = (state == DATA)
                        && (clk_counter == 16'(SHARED_FIRST + bit_count * CLKS_PER_BIT))
                        && (bit_count < 4'd8);
        ...
        case (state)
            ...
            START: begin
                if (SHARED_BAUD) begin
                    if (clk_counter == 16'(HALF_CLKS) && serial_rx == 1'b1)
                        next_state = IDLE;
                    else if (clk_counter == 16'(SHARED_FIRST - 1))
                        next_state = DATA;
                end else begin
                    if (clk_counter == 16'(HALF_CLKS) && serial_rx == 1'b1)
                        next_state = IDLE;
            ...
            STOP: begin
                if (SHARED_BAUD) begin
                    if (clk_counter >= 16'(SHARED_FIRST + 8 * CLKS_PER_BIT)) begin
                        if (serial_rx == 1'b1)
